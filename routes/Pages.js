...
router.get('/teams-leaderboard', checkAuthenticated, (req, res) => {
    ...
    try {
        ...
        const befehl = `SELECT SUM(users.punkte) AS punkte, team.name as name
        FROM users
        ...
        db.query(befehl, (error, results) => {
            ...
            console.log('Daten erfolgreich abgefragt:', results);
            // Sende die Ergebnisse als JSON.
            res.render("teams-leaderboard", { punkte: results[0].punkte , name: results[0].name });
        });
    } catch (error) {
        console.error('Unbehandelter Fehler:', error);
        res.status(500).json({ error: 'Interner Serverfehler' });
...
router.get('/start', checkAuthenticated, (req, res) => {
    ...
    try {
        ...
        switch (befehl) {
            ...
            case "2":
                ...
                if (werte == "Gesamt") {
                    befehl = "SELECT team.name as name, round(sum(users.punkte) / active) as punkte FROM users join team on users.team_id = team.id WHERE punkte != 0 group by team.name order by punkte desc";
                    break;
                } else if (werte == "Woche") {
            ...
            case "5":
                ...
                break;
            case "6":
                befehl = "SELECT date FROM Spiele;";
                break;
...
router.post('/tipp', checkAuthenticated, async (req, res) => {
    ...
    try {
        ...
        const insertQuery = "INSERT INTO tipp (user_Id, match_id, match_date, home_abbr, away_abbr, home_score, away_score, status, recive_date, date ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, now())";
...
router.post('/tippaendern', checkAuthenticated, async (req, res) => {
    ...
    try {
        ...
        const insertQuery = "UPDATE tipp SET home_score = ?, away_score = ?, date = now() WHERE user_Id = ? AND match_id = ? ";
